...
def gauss(euclidean_d, sigma):
    ...
    for i in range(m):
        for j in range(n):
            ...
            Gauss[i, j] = math.exp(- euclidean_d[i, j] / (2 * (sigma1 ** 2)))
    return Gauss
...
class Grnn_gai():
    ...
    def prepare_data(self):
        ...
        self.trX = feature[0:190, :]
        self.trY = label[0:190, :]
        self.teX = feature[190:200, :]
        self.teY = label[190:200, :]
...
if __name__ == "__main__":
    fbest, xbest = BWO.minimize(BWO.functhing, bounds=[(0, 1)], maxiter=20, disp=True)
